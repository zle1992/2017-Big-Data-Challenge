...
def feature(path,str)
    ...
    result = rdd.map(lambda lineline.split( ))
    
    sqlContext = SQLContext(sc)
    column_count = len(result.collect()[0])
    if column_count==4
...
def get_speed(readpath,model)
    ...
    train['data_x_return'] = train['delt_x'].apply(
        lambda x np.where(x0)[0].shape[0] if x.shape[0]0 else 0)
    train['data_x_return'] = train['data_x_return'].apply(
        lambda x 0 if x0 else 1)
    # train['data_x_return'] = train['delt_x'].apply(
    #     lambda x np.where(x0)[0].shape[0]0 if x.shape[0]0 else False)
    ...
    train['speed_x_beyond_index'] = train['speed_x'].map(
        lambda x np.where(np.logical_or.reduce(((x  x.mean() + 2x.std()),(x  x.mean() - 2x.std())))
    ...
    train = train.drop(train.columns[112], axis=1)
